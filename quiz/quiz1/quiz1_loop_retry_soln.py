...
for course in courses:
  print(f"{course['title']} : {course['time']} hour")
...
# You will need to keep a running total of how many hours left for each day before increment of teh marchdate value. 
...
for course in courses:
  print(f"Mar {marchdate} : {course['title']} : {course['time']} hour")
  hourofdayleft -= course['time']
  if hourofdayleft <= 0: 
    marchdate +=1
    hourofdayleft +=8
...
for course in courses:
  print(f"Mar {marchdate} ({dayofweektuple[(marchdate+1)%7]}): {course['title']} : {course['time']} hour")
  hourofdayleft -= course['time']
  if hourofdayleft <= 0: 
    marchdate +=1
    hourofdayleft +=8
