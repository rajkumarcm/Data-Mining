...
# filepath = os.path.join( os.getcwd()  ,'GSS_demographics/GSS_xlsx.csv')
# df = pd.read_csv(filepath)  

# df = pd.read_csv('GSS_demographics/GSS_xlsx.csv', index_col=['id','year'])  
# df = dm.api_dsLand('GSS_demographics_xlsx')
df = dm.api_dsLand('GSS_demographics_xlsx', ['id','year'] )

print("\nReady to continue.")

#%%
# Save it locally for re-loading if needed
# import os
import os
os.getcwd()  # make sure you know what folder you are on 
#%%
os.chdir('.') # do whatever you need to get to the right folder, 
df.to_csv('GSS_demographics_xlsx.csv')  
...
df['childs_convert'] = df.childs.map(lambda x: np.nan if x.strip()=='Dk na' else '8' if x.strip()=='Eight or more' else x )
